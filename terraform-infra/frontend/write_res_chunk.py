...
const STATUS_COLORS = { running:"#00d4aa", active:"#00d4aa", available:"#00d4aa", stopped:"#f59e0b", pending:"#3b82f6", provisioning:"#a78bfa", failed:"#f43f5e" }
...
export default function Resources() {
  ...
  const [s3Buckets, setS3]          = useState([])
  ...
  const fetchAll = useCallback(async () => {
    try {
      ...
      setVms(vr.data)
      setRequests(rr.data.filter(r => !["completed","rejected"].includes(r.status)))
      setS3(sr.data)
      setVpcs(vpcr.data)
      setLambdas(lr.data)
      setLbs(lbr.data)
  ...
  function unlock(type) {
    ...
    if (code.toUpperCase() === ACCESS_CODE) return true
  ...
  async function handleS3Create() {
    if (!s3Form.name.trim()) { setS3Error("Name required"); return }
    setS3C(true); setS3Error("")
    try {
      const { createBucket } = await import("../api/api")
      await createBucket(s3Form)
      setSuccess("Bucket created")
      setS3Modal(false)
      setS3Form({ name:"", region:"ap-south-1", versioning:false, public:false, encryption:"AES256" })
      fetchAll()
      setTimeout(() => setSuccess(""), 3000)
    } catch(e) { setS3Error(e.response?.data?.detail||e.message) }
    finally { setS3C(false) }
  }

  const dataMap = {
    ec2:     vms.map(v => ({ id:v.id, label:v.name, status:v.state, region:v.region, detail:v.instance_type,
      actions: <div style={{display:"flex",gap:"6px"}}>
        {v.state==="stopped" && <button onClick={()=>handleStart(v)} disabled={actionId===v.id+"-start"} style={{padding:"3px 10px",borderRadius:"6px",fontSize:"11px",fontWeight:"500",cursor:"pointer",border:"1px solid #00d4aa40",background:"#00d4aa15",color:"#00d4aa"}}>{actionId===v.id+"-start"?"...":"Start"}</button>}
        {v.state==="running" && <button onClick={()=>handleStop(v)}  disabled={actionId===v.id+"-stop"}  style={{padding:"3px 10px",borderRadius:"6px",fontSize:"11px",fontWeight:"500",cursor:"pointer",border:"1px solid #f59e0b40",background:"#f59e0b15",color:"#f59e0b"}}>{actionId===v.id+"-stop"?"...":"Stop"}</button>}
      </div>
    })),
    s3:      s3Buckets.map(b => ({ id:b.name, label:b.name, status:"active", region:b.region, detail:new Date(b.created).toLocaleDateString("en-IN"),
      actions: <button onClick={()=>handleDeleteBucket(b.name)} style={{padding:"3px 10px",borderRadius:"6px",fontSize:"11px",fontWeight:"500",cursor:"pointer",border:"1px solid #f43f5e40",background:"#f43f5e15",color:"#f43f5e"}}>Delete</button>
    })),
    vpc:     vpcs.map(v    => ({ id:v.id,   label:v.name||v.id, status:"available", region:v.region, detail:v.cidr,  actions:null })),
    lambda:  lambdas.map(l => ({ id:l.name, label:l.name,       status:"active",    region:l.region, detail:l.runtime+" - "+l.size_kb+" KB", actions:null })),
    elb:     lbs.map(lb    => ({ id:lb.name,label:lb.name,      status:lb.state,    region:lb.region,detail:lb.type, actions:null })),
    request: requests.map(r=> ({ id:r.id,   label:r.resource_name, status:r.status, region:"--",    detail:r.resource_type+" by "+r.username, actions:null })),
  }
    ...
    <div style={{ padding:"28px", background:bg, minHeight:"100vh", transition:"all 0.3s" }}>
      ...
      <div style={{ marginBottom:"24px" }}>
        ...
        <p style={{ fontSize:"13px", color:muted, marginTop:"4px" }}>
          {vms.length} EC2 - {s3Buckets.length} S3 - {vpcs.length} VPC - {lambdas.length} Lambda - {lbs.length} LB
        ...
        <div style={{ position:"fixed", top:0, left:0, right:0, bottom:0, background:"rgba(0,0,0,0.55)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:1000, padding:"20px" }}>
          <div style={{ background:surface, borderRadius:"14px", border:"1px solid "+border, width:"100%", maxWidth:"440px" }}>
            ...
            <div style={{ padding:"18px 24px", display:"flex", flexDirection:"column", gap:"12px" }}>
              ...
              <div>
                <label style={{ display:"block", fontSize:"12px", fontWeight:"500", color:muted, marginBottom:"6px" }}>Encryption</label>
